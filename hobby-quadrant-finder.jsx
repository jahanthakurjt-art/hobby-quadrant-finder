      ...
      <div className="hq-layout">
        ...
        <main className="hq-chart-wrap">
          ...
          <div className="hq-chart-row">
            ...
            <div className="hq-chart" onClick={() => setSelected(null)}>
              ...
              {/* ---- Popover anchored at the clicked chip ---- */}
              {selected && (() => {
                const anchor = placed.find((p) => p.name === selected.name);
                const ax = anchor ? anchor.x : 50;
                const ay = anchor ? anchor.y : 50;
                const below = ay < 45; // open downward when chip is near the top
                const left = Math.max(isMobile ? 30 : 16, Math.min(isMobile ? 70 : 84, ax));
                return (
                  <div
                    className={`hq-pop${below ? " below" : ""}`}
                    style={{ left: `${left}%`, top: `${ay}%` }}
                    onClick={(e) => e.stopPropagation()}
                  >
                    <button
                      type="button" className="hq-pop-close"
                      onClick={() => setSelected(null)} aria-label="Close"
                    >×</button>
                    <div className="hq-pop-name">{selected.name}</div>
                    <div className="hq-pop-why">{explain(selected, u)}</div>
                    <div className="hq-pop-actions">
                      ...
                      <button
                        type="button"
                        className={`hq-pop-save${inShortlist(selected.name) ? " saved" : ""}`}
                        onClick={() => toggleShortlist(selected.name)}
                      >
                        {inShortlist(selected.name) ? "★" : "☆"}
                      </button>
                    </div>
                  </div>
                );
...
.hq-pop {
  ...
  width: 236px;
  transform: translate(-50%, calc(-100% - 16px)); /* above the chip */
  ...
  animation: hq-pop-in .16s ease-out;
  cursor: default;
}
.hq-pop.below { transform: translate(-50%, 16px); } /* below when chip is near the top */
@keyframes hq-pop-in {
...
.hq-pop-close:hover { color: #E8ECF4; }
.hq-pop-name { font-size: 16px; font-weight: 600; margin-bottom: 4px; padding-right: 16px; white-space: normal; }
.hq-pop-why { font-size: 12.5px; color: #93A0B8; line-height: 1.5; margin-bottom: 12px; white-space: normal; font-family: 'Space Grotesk', sans-serif; }
.hq-pop-actions { display: flex; gap: 8px; align-items: stretch; }
.hq-pop-go {
  ...
  font-family: 'IBM Plex Mono', monospace; font-size: 11.5px; color: #5EDBA5;
}
.hq-pop-go:hover { box-shadow: 0 0 16px rgba(94,219,165,.4); }
.hq-pop-save {
  background: none; border: 1px solid rgba(232,180,79,.6); border-radius: 10px;
  padding: 8px 12px; cursor: pointer;
  font-size: 14px; color: #E8B44F;
}
.hq-pop-save.saved { background: rgba(232,180,79,.16); }
.hq-pop-save:hover { box-shadow: 0 0 14px rgba(232,180,79,.3); }
...
@media (max-width: 700px) {
  ...
  .hq-pop { width: 200px; padding: 12px 13px; }
  .hq-pop-name { font-size: 14.5px; }
  .hq-pop-why { font-size: 11.5px; }
